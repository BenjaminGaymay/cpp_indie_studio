...
int Indie::Core::createRandMap(std::string name, size_t x, size_t y)
{
...
int Indie::Core::editMapEvents()
{
	...
	if (m_event.MouseState.LeftButtonDown == true) {
		...
		int x = int((m_event.MouseState.Position.X - 362) / BLOCK_SIZE);
		int y = int((m_event.MouseState.Position.Y - 12) / BLOCK_SIZE);
